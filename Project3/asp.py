...
def compute_heuristic_manhattan(l, ex, ey):
    arr = (np.zeros(l.shape))
    for i in range(l.shape[0]):
        for j in range(l.shape[1]):
            arr[i][j] = (abs(ex-i)+abs(ey-j))
    return arr
...
def get_children(x,y,b,visited,parent):
    ...
    if(x+1<b.shape[0]) and b[x+1, y]!= 0 and visited[(x+1,y)] == False:
        children.append((x+1,y))
        parent[(x+1,y)] = (x,y)

    if(y+1<b.shape[1]) and b[x,y+1] != 0 and visited[(x,y+1)] == False:
        children.append((x,y+1))
        parent[(x,y+1)] = (x,y)
        
    if(x-1>=0) and b[x-1, y] != 0 and visited[(x-1,y)] == False:
        children.append((x-1,y))
        parent[(x-1,y)] = (x,y)
        
    if(y-1>=0) and b[x, y-1] != 0 and visited[(x,y-1)] == False:
        children.append((x,y-1))
        parent[(x,y-1)] = (x,y)
...
# l - actual gridworld (with info), h - heuristic to goal of grid,  
#i.j - position of start, ex.ey - node with highest prob, b - knowledge base
def astarplan(l,h,i,j,ex,ey,b):
    ...
    while(i<l.shape[0] and j<l.shape[1]):   
        ...
        if((i,j) == (ex,ey)): #Goal
            ...
            #Add initial to path
            #path.append((si,sj))  #@ADITYA: DO WE REMOVE THIS?!!
