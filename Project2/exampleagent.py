...
'''
This program contains the code for example agent i.e. Agent 3. 
Variables:
...
class exampleAgent:
    ...
    def isBlocked(self,pos):
        if self.grid[pos[0]][pos[1]]==-1:
            ...
            self.cellnworld[2][pos[0]][pos[1]]=0
            return 0
    ...
    def checkNeighbor(self,pos):
        if self.cellnworld[1][pos[0]][pos[1]]==0:
                ...
                for j in range(-1,2):
                    if pos[1]+j>=0 and pos[1]+j<self.n:
                        if self.cellnworld[2][pos[0]+i][pos[1]+j]==-1:
                            countb+=1
                        elif self.cellnworld[2][pos[0]+i][pos[1]+j]==0:
                            counte+=1
    ...
    exploreAgent()-> This function explores the computed path as follows:
                        ...
                        - check whether q is blocked in actual gridworld, if yes, update knowledge base variables and return False and parent of q i.e. p
...
for i in range(10):
    ...
    ea=exampleAgent(grid, start, end, h, n)
    ea.mainAgent()
    print(ea.count1)
    ea.finalRun()
    print(ea.count2)
